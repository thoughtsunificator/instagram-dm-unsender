...
test("IDMU disablePointerEvents", async t => {
	const messagesWrapperElement = createMessagesWrapperElement(t.context.document)
	t.context.document.body.append(messagesWrapperElement)
	const idmu = new IDMU(t.context.window)
	idmu.disablePointerEvents()
	t.is(idmu.uipi.uiComponent.identifier.uiMessagesWrapper.root.style.pointerEvents, "none")
})

test("IDMU enablePointerEvents", async t => {
	const messagesWrapperElement = createMessagesWrapperElement(t.context.document)
	t.context.document.body.append(messagesWrapperElement)
	const idmu = new IDMU(t.context.window)
	idmu.disablePointerEvents()
	...
	t.is(idmu.uipi.uiComponent.identifier.uiMessagesWrapper.root.style.pointerEvents, "")
})
