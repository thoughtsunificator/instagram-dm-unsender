...
test("UnsendThreadMessagesBatchStrategy pointerEvents", async t => {
	const strategy = new UnsendThreadMessagesBatchStrategy(t.context.idmu)
	const messagesWrapperElement = findMessagesWrapperStrategy(t.context.window)
	const promise = strategy.run(1)
	t.is(messagesWrapperElement.style.pointerEvents, "none")
	await promise
	t.is(messagesWrapperElement.style.pointerEvents, "")
})

test("UnsendThreadMessagesBatchStrategy pointerEvents stop", async t => {
	const strategy = new UnsendThreadMessagesBatchStrategy(t.context.idmu)
	const messagesWrapperElement = findMessagesWrapperStrategy(t.context.window)
	strategy.run(1)
	t.is(messagesWrapperElement.style.pointerEvents, "none")
