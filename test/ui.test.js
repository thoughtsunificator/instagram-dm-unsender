...
test("UI disablePointerEvents", async t => {
	const messagesWrapperElement = createMessagesWrapperElement(t.context.document)
	const ui = new UI(t.context.window)
	ui.identifier.uiMessagesWrapper = new UIWrapper(messagesWrapperElement)
	t.context.document.body.append(messagesWrapperElement)
	t.is(ui.identifier.uiMessagesWrapper.root.style.pointerEvents, "")
	ui.disablePointerEvents()
	t.is(ui.identifier.uiMessagesWrapper.root.style.pointerEvents, "none")
})

test("UI enablePointerEvents", async t => {
	const messagesWrapperElement = createMessagesWrapperElement(t.context.document)
	const ui = new UI(t.context.window)
	ui.identifier.uiMessagesWrapper = new UIWrapper(messagesWrapperElement)
	t.context.document.body.append(messagesWrapperElement)
	ui.identifier.uiMessagesWrapper.root.style.pointerEvents = "none"
	ui.identifier.uiMessagesWrapper.enablePointerEvents()
	t.is(ui.identifier.uiMessagesWrapper.root.style.pointerEvents, "")
})
